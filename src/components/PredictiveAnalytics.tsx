      ...
      {predictionsData && predictionsData.length > 0 && (
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '8px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            Live Azure ML Predictions by Roadway (30s Cycle)
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', maxHeight: '180px', paddingRight: '4px' }}>
            {predictionsData.map((pred, i) => (
              <div key={pred.road_name || i} style={{ background: 'rgba(15, 23, 42, 0.4)', border: '1px solid rgba(255,255,255,0.04)', padding: '10px 12px', borderRadius: '8px', fontSize: '11px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                  <span style={{ fontWeight: 'bold', color: 'var(--text-primary)', fontSize: '12px' }}>{pred.road_name}</span>
                  <span className="badge-ui" style={{ background: 'rgba(255,255,255,0.05)', color: 'var(--text-muted)', fontSize: '9px', textTransform: 'capitalize' }}>
                    {pred.weather || 'Sunny'}
                  </span>
                </div>
                
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', color: 'var(--text-secondary)' }}>
                  <div>
                    <div style={{ fontSize: '9px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Vehicle Flow</div>
                    <div style={{ fontWeight: '500', marginTop: '2px' }}>
                      {pred.current_vehicle_count} ➔ <span style={{ color: 'var(--accent-purple)', fontWeight: 'bold' }}>{pred.predicted_vehicle_count}</span>
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: '9px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Avg Speed</div>
                    <div style={{ fontWeight: '500', marginTop: '2px' }}>
                      {pred.current_speed} ➔ <span style={{ color: 'var(--accent-cyan)', fontWeight: 'bold' }}>{pred.predicted_speed} km/h</span>
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: '9px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Congestion</div>
                    <div style={{ fontWeight: '500', marginTop: '2px' }}>
                      {pred.current_congestion}% ➔ <span style={{ color: pred.predicted_congestion > 70 ? 'var(--traffic-red)' : 'var(--traffic-green)', fontWeight: 'bold' }}>{pred.predicted_congestion}%</span>
                    </div>
                  </div>
                </div>
              </div>
